...
bool CPS_Lane_Reset[6]{ false, false, false, false, false, false };
...
void parseCommand(char recvbuf[])
{
	try
	{
		...
		
		{
			...
			for (int i = 0; i < 6; i++)
			{
				bool tempVal = (bool)byte6[i];
				if (tempVal != CPS_Lane_Reset[i] && tempVal)
				{
					//reset on rising edge.
					CPS_Lane_Completed[i] = false;
					logMessage("CPS Complete Reset on Lane " + std::to_string(i + 1));
				}
				CPS_Lane_Reset[i] = tempVal;
			}
...
void sanitizePointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
	for (auto& p : cloud->points)
	{
		if (!std::isfinite(p.x)) p.x = 0.0f;
		if (!std::isfinite(p.y)) p.y = 0.0f;
		if (!std::isfinite(p.z)) p.z = 0.0f;

		// optional clamp for crazy large magnitudes
		if (std::abs(p.x) > 1e6f) p.x = 0.0f;
		if (std::abs(p.y) > 1e6f) p.y = 0.0f;
		if (std::abs(p.z) > 1e6f) p.z = 0.0f;
	}

	cloud->is_dense = true;   // mark as no NaNs
}
auto fix_layout = [](pcl::PointCloud<pcl::PointXYZ>::Ptr& c) {
	const std::size_t n = c->points.size();
	if (n == 0) { c->width = 0; c->height = 0; c->is_dense = true; return; }

	// If organized layout is unknown or inconsistent, make it unorganized.
	if (c->width == 0 || c->height == 0 || std::size_t(c->width) * std::size_t(c->height) != n) {
		c->width = static_cast<uint32_t>(n);
		c->height = 1;
	}
	c->is_dense = true; // we sanitized NaN/Inf -> 0
	};
pcl::PointCloud<pcl::PointXYZ>::Ptr pointCloud_tilt_transform(pcl::PointCloud<pcl::PointXYZ>::Ptr input, double tilt)//, pcl::PointXYZ center)
{
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_out(new pcl::PointCloud<pcl::PointXYZ>);

	float rad = tilt * float(M_PI) / 180.0f;
	//logMessage("rad: " + std::to_string(rad));
	Eigen::Affine3f A = Eigen::Affine3f::Identity();

	// Example: tilt about X
	A.rotate(Eigen::AngleAxisf(rad, Eigen::Vector3f::UnitX()));
	// Optional: A.translation() << tx, ty, tz;

	sanitizePointCloud(input);
	fix_layout(input);
	/*
	pcl::PointCloud<pcl::PointXYZ>::Ptr nan_out(new pcl::PointCloud<pcl::PointXYZ>);
	std::vector<int> idx;
	pcl::removeNaNFromPointCloud(*input, *nan_out, idx);

	auto has_nan = [](const pcl::PointCloud<pcl::PointXYZ>& c) {
		for (const auto& p : c.points) if (!pcl::isFinite(p)) return true; return false;
		};
	//std::cerr << "before: " << has_nan(*nan_out) << "\n";

	auto finite = [](const pcl::PointXYZ& p) {
		return std::isfinite(p.x) && std::isfinite(p.y) && std::isfinite(p.z)
			&& std::abs(p.x) < 1e6f && std::abs(p.y) < 1e6f && std::abs(p.z) < 1e6f;
		};
	pcl::PointCloud<pcl::PointXYZ>::Ptr cleaned(new pcl::PointCloud<pcl::PointXYZ>);
	cleaned->points.reserve(nan_out->points.size());
	for (const auto& p : nan_out->points)
	{
		if (finite(p)) cleaned->points.push_back(p);
	}
	cleaned->width = (uint32_t)cleaned->points.size(); cleaned->height = 1; cleaned->is_dense = true;
	nan_out.swap(cleaned);
	*/
	//std::cerr << "after: " << has_nan(*nan_out) << "\n";

	//for (int i = 0; i < 30; i++)
	//{
	//	std::cout << nan_out->points.at(i) << std::endl;
	//}

	Eigen::Matrix4f T = A.matrix();

	//logMessage("lets see");
	//std::cout << T << std::endl;

	pcl::transformPointCloud(*input, *cloud_out, T);
...
bool save_to_drive(bool isLeftSensor, int procIndex, cv::Mat oImage, pcl::PointCloud<pcl::PointXYZ> pointCloud, std::string saveBaseName, std::chrono::system_clock::time_point frameTime)
{
	try 
	{
		auto baseFolder = saveBaseName;// cpsSaveFolderName[procIndex];
				...
				while (saveFlag)
				{
					try
					{
						if (tsq.GetQueueLen() > 0)
						{
							...
							bool isLeftSensor = false; std::string saveBaseName;
							int procIndex = -1;
							...
							saveBaseName = std::get<4>(dataTup);
							auto frameTime = std::get<5>(dataTup);

							bool save_res = save_to_drive(isLeftSensor, procIndex, oImage, pointCloud, saveBaseName, frameTime);
							if (!save_res) logMessage("Failed to save data to drive!");
							else logMessage("[Data-Save-Thread] Data saved successfully for " + time_as_name(frameTime) + ", Lane-" + std::to_string(procIndex + 1));
...
int CPS_ProcessFrame(pcl::PointCloud<pcl::PointXYZ>::Ptr input, bool isLeftSensor, pcl::PointXYZ& refChassisPosition, std::string savePath = std::string(""), int LaneNumber = 0)
{
	...
	try
	{
		...
		*/
		...
		//auto coc_time = std::chrono::high_resolution_clock::now();
		get_center_of_cloud(*voxel_filtered, ref(baseMaxX), ref(baseMinX), ref(baseMaxY), ref(baseMinY), ref(baseMaxZ), ref(baseMinZ));
		//logMessage("getCenter Time: " + std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - coc_time).count()) + "ms");
		...
		float sMinY = sMaxY - (sMaxY - (float)baseMinY) * 3 / 4;
		//logMessage("y slice: " + std::to_string(sMinY) + "," + std::to_string(sMaxY));
		pc_passThrough(voxel_filtered, sMinY, sMaxY, "y", pass_filtered_y);
		...
		for (int i = 0; i < 10; i++)
		{
			...
			else if (score.score > maxScore)
			{
				...
				pcl::copyPointCloud(*slice, *maxScoreslice);
			...
			//Added condition
			if (slice->points.size() > 100 && !minPC_condition)
			{
				minPC_condition = true;
				//only look upto next 3 slices.
				if (accounted_for) stop_i = i + 2;
				else stop_i = i + 3;
			}
		}

		
		if (selected_i == -1 && maxScoreslice->points.size() > 100)
		{
			if (savePath != std::string(""))
			{
				if (maxScoreslice->points.size() > 0) pcl::io::savePCDFileBinaryCompressed(savePath + "/cps_backup_selected_slice_z_i_" + std::to_string(maxScore_iter) + "pc_" + std::to_string(maxScoreslice->points.size()) +".pcd", *maxScoreslice);
			}
			//refMinZ = get_valid_min_z(slice);
			...
			int refMinX, refMaxX;
			int refMinY, refMaxY;
			get_center_of_cloud(*maxScoreslice, ref(refMaxX), ref(refMinX), ref(refMaxY), ref(refMinY), ref(refMaxZ), ref(refMinZ));

			get_max_min_z(maxScoreslice, refMaxZ, refMinZ);
								...
								if (dataStream[procIndex].getNextFrame())
								{
...

									{
										...
										*/
										...
										if (abs(chassisPos - LastRD) < 100 && (LastRD != 10000 || chassisPos != 10000))
										{
											if (ChassisStopCounter < 15) ChassisStopCounter++;
											if (ChassisStopCounter >= 15 && !ChassisStopped)
											{
												RDMessage("[CPS-Lane: " + std::to_string(LaneNumber) + "] Chassis Stopped at: " + std::to_string(chassisPos) + "mm", LaneNumber, true);
												ChassisStopped = true;
											}
										}
										else if (abs(chassisPos - LastRD) > 200 && ChassisStopped)
										{
											//Chassis moving
											RDMessage("[CPS-Lane: " + std::to_string(LaneNumber) + "] Chassis Moving!", LaneNumber, true);
											ChassisStopCounter = 0;
											ChassisStopped = false;
										}

										bool saveEnable = false;

										if (abs(chassisPos - LastRD) > 50 && (LastRD != 10000 || chassisPos != 10000) && ChassisStopped)
										{
											if (flicker_count <= flicker_limit) flicker_count++;
											if (flicker_count >= flicker_limit) saveEnable = true;

											RDMessage("[CPS-Lane: " + std::to_string(LaneNumber) + "] Flicker Save - Current RD: " + std::to_string(chassisPos) + "mm" + " , Previous RD: " + std::to_string(LastRD), LaneNumber, true);
...
void CPS_Processing_Debug()
{
	...
	auto total_logHeader = std::string("Filename;TargetDistance");
	ofstream Simfile2;
	std::string sim_result_txt2 = DEBUG_BATCH_SAVE_DIR + "/batch_res.txt";
	Simfile2.open(sim_result_txt2.c_str(), ios::out | ios::app);
	if (!Simfile2.is_open())
	{
