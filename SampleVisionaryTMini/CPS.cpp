...
CPS::CPS(
    ...
    m_minChassisHeight(minChassisHeight),
...
float CPS::detectChassisDistance(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    ...
    // Step 1: Filter by distance range and height
    m_filteredCloud.reset(new pcl::PointCloud<pcl::PointXYZ>);
    for (const auto& point : cloud->points) {
        // Only keep points within valid distance range and above minimum chassis height
        if (point.z >= m_minDistance && point.z <= m_maxDistance &&
            point.y >= (m_groundHeight + m_minChassisHeight)) {
    ...
    if (bestInliers >= m_minInliers) {
        ...
        if (static_cast<int>(m_distanceHistory.size()) > m_historySize) {
...
void CPS::setMaxDistance(float maxDistance) {
    m_maxDistance = maxDistance;
}

void CPS::setMinDistance(float minDistance) {
    m_minDistance = minDistance;
}

void CPS::setGroundHeight(float groundHeight) {
    m_groundHeight = groundHeight;
}

void CPS::setMinChassisHeight(float minChassisHeight) {
    m_minChassisHeight = minChassisHeight;
}
...
void CPS::setParams(float maxDistance,
    float minDistance,
    float groundHeight,
    float minChassisHeight,
    int historySize,
